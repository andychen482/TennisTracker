...
def get_num_frames(video_file):
    ...
    result = parse.parse('{}\\video\\{}.mp4', video_file)
    if result is None:
        print(f'Failed to parse video file: {video_file}')
        return None
    
    match_dir, rally_id = result
    frame_dir = f'{match_dir}\\frame\\{rally_id}'
    
    print(f'Parsed match_dir: {match_dir}, rally_id: {rally_id}')
